...
class FeedState extends AppState {
  ...
  /// It makes app to view nested Tweets
  ...
  void getDataFromDatabase() {
    try {
      ...
      // for feeds in posts collection
      kDatabase.child('posts').once().then((DataSnapshot snapshot) {
        _feedlist = <FeedModel>[];
    ...
    try {
      ...
      if (model != null) {
        ...
        try {
          ...
          cprint(error, errorIn: 'Post Detail');
        }
        ...
        if (_tweetDetail.replyTweetKeyList != null &&
          ...
          _tweetDetail.replyTweetKeyList.forEach((x) {
            ...
            try {
              ...
              cprint(error, errorIn: 'post Comment');
            }
  ...
  Future<FeedModel> fetchTweet(String postID) async {
    ...
    /// If tweet is not available in feedlist then need to fetch it from firebase
      ...
      // fetch new posts
      var postModel = await kDatabase.child('posts').child(postID).once().then(
        (DataSnapshot snapshot) {
          if (snapshot.value != null) {
            var map = snapshot.value;
            _tweetDetail = FeedModel.fromJson(map);
      ...
      if (model != null && postModel != null) {
        _tweetDetail = model + postModel;
      } else {
        _tweetDetail = postModel;
  ...
  Future<String> createTweet(FeedModel model) async {
    ...
    try {
      ...
      DatabaseReference dbReference = kDatabase.child('posts').push();
  ...
  deleteTweet(String tweetId, TweetType type, {String parentkey}) {
    try {
      // remove new posts
      kDatabase.child('posts').child(tweetId).remove().then((_) {
  ...
  updateTweet(FeedModel model) async {
    await kDatabase.child('posts').child(model.key).set(model.toJson());
    // await kDatabase.child('tweet').child(model.key).set(model.toJson());
  }

  /// Remove like on a Post
  removeDislike({FeedModel tweet, String userId}) {
    try {
      if (tweet.dislikeList.length > 0 &&
          tweet.dislikeList.any((id) => id == userId)) {
        tweet.dislikeList.removeWhere((id) => id == userId);
        tweet.dislikeCount -= 1;
      }
    } catch (error) {
      cprint(error, errorIn: 'removing dislike');
    }
  }

// Remove dislike on Post
  removeLike({FeedModel tweet, String userId}) {
    try {
      ...
      cprint(error, errorIn: 'removing like');
    }
  }
  ...
  addLikeToTweet(FeedModel tweet, String userId) {
    try {
      ...
      if (tweet.likeList != null &&
        ...
        removeDislike(tweet: tweet, userId: userId); //remove dislike
        tweet.likeList.add(userId);
        tweet.likeCount += 1;
      }
      // update likelist of a new posts
      kDatabase
          .child('posts')
          .child(tweet.key)
          .child('likeList')
      ...
      kDatabase
          ...
          .child('likeList')
          .set(tweet.likeList);
      // update dislikelist of a new posts
      kDatabase
          .child('posts')
          .child(tweet.key)
          .child('dislikeList')
          .set(tweet.dislikeList);
      // update likelist of a tweet
      kDatabase
          .child('tweet')
          .child(tweet.key)
  ...
  /// Add/Remove dislike on a Post
  /// [postId] is tweet id, [userId] is user's id who like/unlike Tweet
  addDislikeToTweet(FeedModel tweet, String userId) {
    try {
      if (tweet.dislikeList != null &&
          tweet.dislikeList.length > 0 &&
          tweet.dislikeList.any((id) => id == userId)) {
        // If user wants to undo/remove his like on tweet
        tweet.dislikeList.removeWhere((id) => id == userId);
        tweet.dislikeCount -= 1;
        ...
        removeLike(tweet: tweet, userId: userId); //remove dislike
        tweet.dislikeList.add(userId);
        tweet.dislikeCount += 1;
      }
      ...
      // UserModel owner can see notification on notification page
      kDatabase.child('notification').child(tweet.userId).child(tweet.key).set({
        'type': tweet.dislikeList.length == 0
            ? null
            : NotificationType.Dislike.toString(),
        'updatedAt': tweet.dislikeList.length == 0
            ? null
            : DateTime.now().toUtc().toString(),
      });
    } catch (error) {
      cprint(error, errorIn: 'addDislikeToPost');
    }
  }
  ...
  Future<String> addcommentToPost(FeedModel replyTweet) async {
    try {
      ...
      if (_tweetToReplyModel != null) {
        ...
        //   updateTweet(tweet);
        // });
        DatabaseReference ref = kDatabase.child('posts').push();
        await ref.set(json);
  ...
  _onTweetRemoved(Event event) async {
    ...
    try {
      ...
      if (deletedTweet.imagePath != null && deletedTweet.imagePath.length > 0) {
        deleteFile(deletedTweet.imagePath, 'postImages');
      }
...
// post video view count
      ...
      kDatabase
        ...
        if (snapshot.exists) {
          try {
            ...
            kDatabase.child('posts').child(tweet.key).update({
              'viewCount': viewCount,
            });
          } catch (error) {
            cprint(error, errorIn: 'view Count Added');
          }
        } else {
          // try {
          try {
            ...
            kDatabase.child('tweet').child(tweet.key).update({
              'viewCount': viewCount,
            });
          } catch (error) {
            cprint(error, errorIn: 'view Count Added');
  ...
  // report posts
  reportPost(FeedModel tweet) async {
    EasyLoading.show(status: 'Doing some checks...');

    int reportCounts;
    User user = FirebaseAuth.instance.currentUser;
    SocialMediaServices _services = SocialMediaServices();
    DocumentSnapshot document = await _services.reportPost.doc(tweet.key).get();
    if (document.exists) {
      reportCounts = document['reportCounts'];
      if (document['reports'].contains(user.uid)) {
        
        EasyLoading.showSuccess(
            'You\'ve already submitted a report against this content');

        // _services.reportPost.doc(tweet.key).update({
        //   'reports': FieldValue.arrayRemove([user.uid]),
        //   'reportCounts': reportCounts - 1,
        // });
      } else {
        _services.reportPost.doc(tweet.key).update({
        ...
        EasyLoading.showSuccess('Report submitted successfully to Wiwa Admin');
      }
    } else {
      EasyLoading.show(status: 'Creating report');
      _services.reportPost.doc(tweet.key).set({
        'reports': [user.uid],
        'reportCounts': 1,
        'reportedContent': tweet.description,
        'contentOwner': tweet.user.userName,
        'contentOwnerId': tweet.userId
      });
      EasyLoading.showSuccess('Report submitted successfully to Wiwa Admin');
    }
  }
